...
import { MapPinIcon, UsersIcon, DollarSignIcon } from "./EventIcons";
    ...
    >
      <div className="flex flex-col gap-5 lg:gap-6">
        <div className="flex gap-3 lg:gap-5 flex-wrap">
          <div
            className={`border ${
              theme === "gold" ? "border-yellow-dark" : "border-green-light"
...
const RedLayout: React.FC<{
  ...
  config: any;
}> = ({ sections, specialDescription, config }) => {
  return (
    <div className="border-t border-gray-200 px-6 lg:px-12 py-8 lg:py-12 flex flex-col gap-8 lg:gap-12">
      ...
      <div className="flex flex-col gap-4 lg:gap-5">
        ...
        >
          {sections[0]?.title || "Leadership Development Program"}
        </p>
        <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        {/* Left box - Meals included (dark red bg) */}
        <div className={`${config.boxBg} rounded-3xl p-6 lg:p-8 shadow-md`}>
          <p
            className={`font-semibold text-xl lg:text-2xl text-white mb-4 lg:mb-6`}
          >
            Meals included
          </p>
          <ul
            className={`font-normal text-base lg:text-lg text-white space-y-2`}
          >
        ...
        {/* Right box - What you will gain (light bg) */}
        <div className="bg-red-light rounded-3xl p-6 lg:p-8 shadow-md">
          <p
            className={`font-semibold text-xl lg:text-2xl ${config.headingText} mb-4 lg:mb-6`}
          >
            What You will gain
          </p>
          <ul
      ...
      <div className="bg-red-light rounded-3xl p-6 lg:p-8 shadow-md">
        <p
          className={`font-semibold text-xl lg:text-2xl ${config.headingText} mb-6`}
        >
        ...
        <div className="flex flex-col md:flex-row gap-6 lg:gap-12">
          <div className="flex-1">
            ...
            >
              "I learned that leadership isn't about having all the answers –
              it's about asking the right questions and helping others find
              their own solutions."
            </p>
            <p
              className={`font-normal text-sm lg:text-base ${config.specialText} mt-3 lg:mt-4`}
            >
              - Ana, Age 16
            </p>
          </div>
          ...
          <div className="flex-1">
            <p
              className={`font-normal text-base lg:text-lg ${config.specialText}`}
            >
              "The retreat helped me understand that my quiet leadership style
              is just as valuable as being outgoing. I feel more confident in
              who I am."
            </p>
            <p
              className={`font-normal text-sm lg:text-base ${config.specialText} mt-3 lg:mt-4`}
...
const GreenLayout: React.FC<{
  ...
  config: any;
}> = ({ sections, specialDescription, timeline, config }) => {
  return (
    <>
      <div className="px-6 lg:px-12 py-8 lg:py-12 flex flex-col lg:flex-row gap-8 lg:gap-12 xl:gap-20">
        ...
        <div
          className={`${config.boxBg} ${
            config.boxBorder ? `border-2 ${config.boxBorder}` : ""
      ...
      <div className="px-6 lg:px-12 py-8 lg:py-12 grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        {/* Helper function to determine box style based on content */}
        {sections.slice(1).map((section, idx) => {
          // Identify if it's the "Meals included" section (Green solid box)
          const isMeals = section.title.toLowerCase().includes("meals");
          const boxStyle = isMeals
            ? "bg-[#01DA81] text-white rounded-[30px]"
            : "bg-[#effdf4] text-green-normal rounded-3xl"; // Light green for others

          return (
            <div key={idx} className={`${boxStyle} p-6 lg:p-8 shadow-md`}>
              <h3
                className={`font-semibold text-xl lg:text-2xl mb-4 lg:mb-6 ${
                  isMeals ? "text-white" : config.headingText
                }`}
              >
                {section.title}
              </h3>
              {section.items ? (
                <ul
                  className={`font-normal text-base lg:text-lg space-y-2 ${
                    isMeals ? "text-white" : config.specialText
                  }`}
                >
...
const GoldLayout: React.FC<{
  ...
  return (
    <>
      ...
      {/* Bottom Sections - 3 Columns with Boxes */}
...
const EventCard: React.FC<EventCardProps> = (props) => {
  const { theme, sections, specialDescription, timeline } = props;
  const config = themeConfigs[theme];
